...
您可以透過在題目 JSON 物件中加入 `imageUrl` 欄位來引入圖片。此欄位支援兩種路徑：

1.  **相對路徑 (建議)**: 在您的專案根目錄下建立一個 `images` 資料夾，並將所有題目圖片放入其中。接著，`imageUrl` 的值應設為 `images/您的圖片檔名.png`。這種方式讓專案可以離線運作且方便管理。
2.  **絕對網址**: 您也可以使用圖片的**完整網址** (例如，上傳至 Firebase Storage、Imgur 或其他圖床服務的連結)。

**範例：一個包含圖片的單字題 (使用相對路徑)**

假設您已在專案中建立 `images/placeholder.png` 檔案：
...
[
  {
    ...
    "level": "intermediate",
    "imageUrl": "ImagesEphedrine_450x450.png",
    "question": "This is a _______ image, used when the final image is not yet available.",
    "options": ["placeholder", "permanent", "dynamic", "vector"],
    "correct": 0,
    "explanation": "A placeholder image holds a place for the final content."
  }
]
```

系統會自動偵測到 `imageUrl` 欄位，並在測驗畫面和答案回顧頁面中顯示對應的圖片。
